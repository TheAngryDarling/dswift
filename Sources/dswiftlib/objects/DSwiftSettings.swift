//
//  DSwiftSettings.swift
//  dswiftlib
//
//  Created by Tyler Anger on 2019-07-21.
//

import Foundation
import XcodeProj
import BasicCodableHelpers
import SwiftPatches
import VersionKit

/// Settings structure for the config file
public struct DSwiftSettings {
    public enum SettingsErrors: Swift.Error, CustomStringConvertible {
        case unableToConvertStringToData(string: String, objectType: String, encoding: String.Encoding)
        
        public var description: String {
            switch self {
                case .unableToConvertStringToData(string: _,
                                                  objectType: let objType,
                                                  encoding: let encoding):
                    return "Unable to convert '\(objType)' into data using encoding '\(encoding)'"
            }
        }
    }
    public enum CodingKeys: String, CodingKey {
        case swiftPath
        case xcodeResourceSorting
        case license
        case readme
        case whenToAddBuildRules
        case generateXcodeProjectOnInit
        case regenerateXcodeProject
        case repository
        case authorName
        case authorContacts
        case lockGenFiels = "lockGeneratedFiles"
        case verboseMode
        case includeGeneratedFilesInXcodeProject
        case includeSwiftLintInXcodeProjectIfAvailable
        case includeSwiftLintInXcodeProject
        case autoInstallMissingPackages
        case defaultPackageInitToolsVersion
        case defaultGitIgnore
        case gitIgnoreAdditions
    }
    /// Sorting Option when Sorting XcodeProject resources
    public enum XcodeProjectFileResourceSorting: String, Codable {
        case none
        case sorted
    }
    /// Structure containing indicators for the different project types if the XcodeProject
    /// should be generated
    public struct GenerateXcodeProjectOnInit: Collection {
        
        public typealias Index =  Dictionary<String, Bool>.Index
        public typealias Element = Dictionary<String, Bool>.Element
        
        private var types: [String: Bool]
        public var isEmpty: Bool { return self.types.isEmpty }
        
        public var startIndex: Index { return self.types.startIndex }
        public var endIndex: Index { return self.types.endIndex }
        
        public init() { self.types = [:] }
        
        public subscript(name: String) -> Bool? {
            get {
                return self.types[name.lowercased()]
            }
            set {
                let lowName = name.lowercased()
                // If we have a real value, the we will try and
                // remove any 'all' type
                if lowName != "all" && newValue != nil {
                    self.types["all"] = nil
                }
                self.types[lowName] = newValue
            }
        }
        public subscript(position: Index) -> Element {
            return self.types[position]
        }
        
        public func index(after i: Index) -> Index {
            return self.types.index(after: i)
        }
        
        public func canGenerated(forName name: String) -> Bool {
            if let val = self.types["all"] { return val }
            
            var name = name.lowercased()
            if name == "sysmod" { name = "system-module" }
            
            if let val = self.types[name] { return val }
            if let val = self.types["default"] { return val }
            
            return false
            
        }
    }
    
    /// Contact information for the developer
    public enum Contact {
        case email(String)
        case address(String)
        case phone(String)
        case url(String)
        /// Indicator if this is an email contact
        public var isEmail: Bool {
            guard case Contact.email(_) = self else { return false }
            return true
        }
        /// Indicator if this is an address contact
        public var isAddress: Bool {
            guard case Contact.address(_) = self else { return false }
            return true
        }
        /// Indicator if this is an phone number contact
        public var isPhone: Bool {
            guard case Contact.phone(_) = self else { return false }
            return true
        }
        /// Indicator if this is an web url contact
        public var isURL: Bool {
            guard case Contact.url(_) = self else { return false }
            return true
        }
        /// The string value of the contact
        public var string: String {
            switch self {
                case .email(let string): return string
                case .address(let string): return string
                case .phone(let string): return string
                case .url(let string): return string
            }
        }
    }
    
    /// The difference supported license types
    public enum License {
        case none
        case file(URL)
        case apache2_0
        case gnuGPL3_0
        case gnuAGPL3_0
        case gnuLGPL3_0
        case mozilla2_0
        case mit
        case unlicense
        /// Returns an array of all standard licenses available (This excludes the file option)
        public static var allStandardLicenses: [License] {
            return [.apache2_0,
                    .gnuGPL3_0,
                    .gnuAGPL3_0,
                    .gnuLGPL3_0,
                    .mozilla2_0,
                    .mit,
                    .unlicense]
        }
        /// Is there no license
        public var isNone: Bool {
            guard case .none = self else { return false }
            return true
        }
        /// Is the license from a file
        public var isFile: Bool {
            guard case .file(_) = self else { return false }
            return true
        }
        
        /// Returns the display name of the license, otherwise an empty string if no display name is available
        public var displayName: String {
            switch self {
                case .apache2_0: return "Apache License v2.0"
                case .gnuGPL3_0: return "GNU General Public License v3.0"
                case .gnuAGPL3_0: return "GNU Affero General Public License v3.0"
                case .gnuLGPL3_0: return "GNU Lesser General Public License v3.0"
                case .mozilla2_0: return "Mozilla Public License 2.0"
                case .mit: return "MIT"
                case .unlicense: return "The Unlicense"
                default: return ""
            }
        }
        /// Name of the ReadME badge or empty string if there isn't one
        public var badgeName: String {
            switch self {
                case .apache2_0: return "Apache 2.0"
                case .gnuGPL3_0: return "GPL v3"
                case .gnuAGPL3_0: return "AGPL v3"
                case .gnuLGPL3_0: return "LGPL v3"
                case .mozilla2_0: return "MPL 2.0"
                case .mit: return "MIT"
                case .unlicense: return "Unlicense"
                default: return ""
            }
        }
        /// Colour of the ReadME badge or empty string if there isn't one
        public var badgeColour: String {
            switch self {
                case .apache2_0: return "blue"
                case .gnuGPL3_0: return "blue"
                case .gnuAGPL3_0: return "blue"
                case .gnuLGPL3_0: return "blue"
                case .mozilla2_0: return "brightgreen"
                case .mit: return "yellow"
                case .unlicense: return "blue"
                default: return ""
            }
        }
        
        /// Generate License text for the ReadMe file
        /// - Parameters:
        ///   - authorName: The author of the project
        ///   - date: The date the ReadMe file / License / Copyright was generated
        /// - Returns: Returns the License text for the ReadMe file
        public func readmeText(authorName: String? = nil,
                               date: Date = Date()) -> String {
            switch self {
                case .none: return ""
                case .apache2_0: return Licenses.apache2_0_README(authorName: authorName,
                                                                  date: date)
                case .gnuGPL3_0: return Licenses.gnuGPL3_0_README(authorName: authorName,
                                                                  date: date)
                case .gnuAGPL3_0: return Licenses.gnuAGPL3_0_README(authorName: authorName,
                                                                    date: date)
                case .gnuLGPL3_0: return Licenses.gnuLGPL3_0_README(authorName: authorName,
                                                                    date: date)
                case .mozilla2_0: return Licenses.mozilla2_0_README(authorName: authorName,
                                                                    date: date)
                case .mit: return Licenses.mit_README(authorName: authorName,
                                                      date: date)
                case .unlicense: return Licenses.unlicense_README(authorName: authorName,
                                                                  date: date)
                default: return Licenses.defaultLicenseReadMe(authorName: authorName,
                                                              date: date)
            }
        }
        
    }
    
    /// Developer repository information
    public struct Repository {
        
        public enum CodingKeys: String, CodingKey {
            case serviceName
            case serviceURL
            case repositoryName
        }
        /// Repository Service Name (eg GitHub, GitLab, etc...)
        public let serviceName: String?
        /// URL to the developer repository
        public let serviceURL: URL
        /// Repository name (eg last part of service url)
        public let repositoryName: String?
        /// description build for ReadME file
        public var readMEDescription: String {
            var rtn: String = "["
            if let s = serviceName { rtn += s }
            if let r = repositoryName {
                if rtn != "[" { rtn += " - " }
                rtn += r
            }
            if rtn == "[" { rtn += serviceURL.lastPathComponent }
            rtn += "](\(serviceURL.absoluteString))"
            return rtn
        }
    }
    
    /// Difference location of readme files
    public struct ReadMe: Collection {
        
        /// Readme Type, Either URL or auto-generated
        public enum ReadMeType {
            case url(URL)
            case generated
            
            var url: URL? {
                guard case .url(let val) = self else { return nil }
                return val
            }
        }
        
        public typealias Index =  Dictionary<String, ReadMeType>.Index
        public typealias Element = Dictionary<String, ReadMeType>.Element
        
        private var types: [String: ReadMeType]
        public var isEmpty: Bool { return self.types.isEmpty }
        
        public var startIndex: Index { return self.types.startIndex }
        public var endIndex: Index { return self.types.endIndex }
        /// Create new Empty
        public init() {
            self.types = [:]
            
        }
        public init(_ readMeType: ReadMeType) {
            self.types = ["all": readMeType]
        }
        
        public subscript(name: String) -> ReadMeType? {
            get {
                return self.types[name.lowercased()]
            }
            set {
                let lowName = name.lowercased()
                // If we have a real value, the we will try and
                // remove any 'all' type
                if lowName != "all" && newValue != nil {
                    self.types["all"] = nil
                }
                self.types[lowName] = newValue
            }
        }
        public subscript(position: Index) -> Element{
            return self.types[position]
        }
        
        public func index(after i: Index) -> Index {
            return self.types.index(after: i)
        }
        
        
        /// Get the ReadMeType for the given module type name.
        /// If type 'all' is set then it will return no matter what name is passed
        /// - Parameter name: The name of the module type to look for
        /// - Returns: Returns the ReadMeType OR nil if no type is found for the given module type name
        public func getType(forName name: String) -> ReadMeType? {
            if let val = self.types["all"] { return val }
            
            var name = name.lowercased()
            if name == "sysmod" { name = "system-module" }
            
            if let val = self.types[name] { return val }
            if let val = self.types["default"] { return val }
            
            return nil
            
        }
        /// The response object to the generateReadMe method
        public enum GeneratedReadMeResponse {
            case copyFrom(url: URL, content: String, encoding: String.Encoding)
            case replaceWith(content: String, encoding: String.Encoding)
            
            /// The content of the ReadMe file
            public var content: String {
                switch self {
                    case .copyFrom(url: _, content: let content, encoding: _):
                        return content
                    case .replaceWith(content: let content, encoding: _):
                        return content
                }
            }
            /// The encoding used for the ReadMe file
            public var encoding: String.Encoding {
                switch self {
                    case .copyFrom(url: _, content: _, encoding: let encoding):
                        return encoding
                    case .replaceWith(content: _, encoding: let encoding):
                        return encoding
                }
            }
        }
        /// Errors regarding the generateReadMe method
        public enum GenerateReadMeError: Swift.Error, CustomStringConvertible {
            case fileNotFound(path: String, absPath: String)
            
            public var description: String {
                switch self {
                case .fileNotFound(path: let path, absPath: _):
                    return "Missing ReadMe File '\(path)'"
                }
            }
        }
        
        /// Generate The ReadMe content
        /// - Parameters:
        ///   - modType: The Project Module Type
        ///   - name: The name of the Project
        ///   - dswiftMessage: Indicator if the DSwift Message should be included
        ///   - dswiftInfo: Information about the current version of DSwift
        ///   - currentProjectURL: The URL to the project
        ///   - authorName: The Authors name
        ///   - repository: The Authors Repository Information
        ///   - license: The license for this Project
        ///   - date: The date used as the generated / CopyRight date
        ///   - console: The object used to print to the console
        /// - Returns: Returns a generated ReadMe Response object or nil if no ReadMe object is generated
        public func generateReadMe(for modType: String,
                                   withName name: String,
                                   includeDSwiftMessage dswiftMessage: Bool = true,
                                   dswiftInfo: DSwiftInfo,
                                   currentProjectURL: URL,
                                   authorName: String? = nil,
                                   repository: Repository? = nil,
                                   license: License,
                                   date: Date = Date(),
                                   console: Console = .null) throws -> GeneratedReadMeResponse? {
            
            guard let readmeType = self.getType(forName: modType) else { return nil }
            
            let authorName: String? = authorName ?? XcodeProjectBuilders.UserDetails().displayName
            
            if var rURL = readmeType.url {
                if rURL.isFileURL {
                    let path = rURL.path
                    let absPath = path.fullPath().resolvingSymlinksInPath
                    rURL = URL(fileURLWithPath: absPath)
                    guard FileManager.default.fileExists(atPath: absPath) else  {
                        throw GenerateReadMeError.fileNotFound(path: path, absPath: absPath)
                    }
                }
                
                var encoding: String.Encoding = .utf8
                var readMe = try String.init(contentsOf: rURL, foundEncoding: &encoding)
                
                if let author = authorName, !author.isEmpty {
                    readMe = readMe.replacingOccurrences(of: "{author_name}", with: author)
                }
                
                if let r = repository {
                    if let rName = r.repositoryName {
                        readMe = readMe.replacingOccurrences(of: "{repository_name}", with: rName)
                    }
                    if let sName = r.serviceName {
                        readMe = readMe.replacingOccurrences(of: "{repository_service}", with: sName)
                    }
                    readMe = readMe.replacingOccurrences(of: "{repository_url}",
                                                         with: r.serviceURL.absoluteString)
                }
                
                return .copyFrom(url: readmeType.url!, content: readMe, encoding: encoding)
                
            } else {
                var readmeContents: String = "# \(name)\n\n"
                var packageVersion: String = "4.0"
                let packageFileURL = currentProjectURL.appendingPathComponent("Package.swift")
                if FileManager.default.fileExists(atPath: packageFileURL.path) {
                    do {
                        let src = try String(contentsOf: packageFileURL)
                        let firstLine = String(src.split(separator: "\n").first!)
                        packageVersion = firstLine.replacingOccurrences(of: "// swift-tools-version:", with: "")
                        readmeContents += "![swift >= \(packageVersion)](https://img.shields.io/badge/swift-%3E%3D\(packageVersion)-brightgreen.svg)\n"
                    } catch {}
                }
                readmeContents += "![macOS](https://img.shields.io/badge/os-macOS-green.svg?style=flat)\n"
                readmeContents += "![Linux](https://img.shields.io/badge/os-linux-green.svg?style=flat)\n"
                if !license.isNone && !license.badgeName.isEmpty {
                    
                    let webEscapedName = license.badgeName.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed)!
                    let licenseBadgeStr = "![\(license.badgeName)](https://img.shields.io/badge/License-\(webEscapedName)-\(license.badgeColour).svg?style=flat)"
                    readmeContents += "[\(licenseBadgeStr)](LICENSE.md)\n"
                }
                readmeContents += "\n"
                readmeContents += "<Project description goes here>\n\n"
                
                readmeContents += "## Requirements\n\n"
                if let v = Version.SingleVersion(packageVersion) {
                    let req = XcodeProjectBuilders.DefaultDetailsChoice.swiftVersion(v)
                    if let buildOptions = req.mostCompatible() {
                        readmeContents += "* \(buildOptions.compatibleXcode)+ (If working within Xcode)\n"
                        if let sV = buildOptions.swiftVersion {
                            readmeContents += "* \(sV)+\n"
                        }
                        readmeContents += "\n"
                    }
                }
                
                if dswiftMessage {
                    readmeContents += "> Note: This package used [\(dswiftInfo.moduleName)](\(dswiftInfo.url)) to generate some, or all, of its source code.  While the generated source code should be included and available in this package so building directly with swift is possible, if missing, you may need to download and build with [\(dswiftInfo.moduleName)](\(dswiftInfo.url))\n\n"
                }
                
                readmeContents += "## Usage\n\n"
                readmeContents += "<Usage goes here>\n\n"
                readmeContents += "```swift\n\n"
                readmeContents += "```\n\n"
                readmeContents += "## Dependencies\n\n"
                readmeContents += "<Project dependencies goes here or remove this section>\n\n"
                readmeContents += "## Author\n\n"
                readmeContents += "* **\(authorName ?? XcodeProjectBuilders.UserDetails().displayName!)** - *Initial work* "
                if let r = repository {
                    readmeContents += " - " + r.readMEDescription
                }
                readmeContents += "\n\n"
                
                if !license.isNone {
                    readmeContents += "## License\n\n"
                    readmeContents += license.readmeText(authorName: authorName, date: date) + "\n\n"
                    //readmeContents += "This project is licensed under \(settings.license.displayName) - see the [LICENSE.md](LICENSE.md) file for details.\n\n"
                }
                readmeContents += "## Acknowledgments\n\n"
                readmeContents += "<Acknowledgments go here or remove this section>"
                
                return .replaceWith(content: readmeContents, encoding: .utf8)
                
            }
        }
    }
    
    /// Defines when build rules can be added
    public enum AddingBuildRulesRule: String, Codable {
        /// Always add build rules
        case always
        /// Add build rues when needed
        case whenNeeded
        
        /// Method used to determing if build rules can be added
        /// - Parameters:
        ///   - project: The URL to the project
        ///   - generator: The generators
        /// - Returns: Returns true if build rules can be added otherwise false
        public func canAddBuildRules(_ project: URL,
                                     generator: GroupGenerator) throws -> Bool {
            switch self {
                case .always: return true
                case .whenNeeded: return try generator.containsSupportedFiles(inFolder: project)
            }
        }
    }
    /// Defines the Usability of a an object
    public enum Usability: String, Codable {
        /// Always use
        case always
        /// Use when available
        case whenAvailable
        /// Never use
        case never
    }
    
    /// The default swift path
    public static let defaultSwiftPath: String = "/usr/bin/swift"
    /// The path to the swift to use (Default: DSwiftSettings.defaultSwiftPath)
    public var swiftPath: String
    /// Indictor if the Xcode Project structure should be sorted or not
    public let xcodeResourceSorting: XcodeProjectFileResourceSorting
    /// License settings
    public let license: License
    /// Readme settings
    public let readme: ReadMe
    /// Indicator of when to add build rules
    public let whenToAddBuildRules: AddingBuildRulesRule
    /// Indicator if 'swift packgae generate-xcodeproj' should be executed after 'swift package init'
    public let generateXcodeProjectOnInit: GenerateXcodeProjectOnInit
    /// Indicator if 'swift packgae generate-xcodeproj' should be executed after any 'swift package update'
    public let regenerateXcodeProject: Bool
    /// Developer repository information
    public let repository: Repository?
    /// Author Name for use in README.md file when generated
    public let authorName: String?
    /// Author Contact for use in README.md file when generated
    //public let authorContacts: [Contact]
    /// Lock generated files.  Locks any generated files from being modified
    public let lockGenFiles: Bool
    /// Indicator if we should be in verbose mode
    public let verboseMode: Bool
    /// Indicator if we should be in verbose mode (checks env variable dswiftVerbose and then checks verboseMode property)
    public var isVerbose: Bool {
        guard let envVar = ProcessInfo.processInfo.environment["dswiftVerbose"] else {
            return self.verboseMode
        }
        return (envVar.lowercased() == "true")
    }
    /// Indicator if generated source code files should be included in Xcode Projects
    public let includeGeneratedFilesInXcodeProject: Bool
    /// indicator, if when generating Xcode Project file would install SwifLint Build Phase Script
    public let includeSwiftLintInXcodeProject: Usability
    
    /// Indicator if we should try to auto install any missing system packages from the package manager
    /// when requirements are found within the project, otherwise we will just give a warning
    public let autoInstallMissingPackages: Bool
    /// Set the package tools version to the give value when creating new package if the value is set
    public let defaultPackageInitToolsVersion: String?
    /// Allows for the settings file to override the default gitignore file
    public let defaultGitIgnore: GitIgnoreFile?
    /// Allows for the settings file to add additional rules to the gitignore file
    public let gitIgnoreAdditions: GitIgnoreFile?
    
    /// Create new DSwift Settings Object with default values
    public init() {
        self.swiftPath = DSwiftSettings.defaultSwiftPath
        self.xcodeResourceSorting = .none
        self.license = .none
        self.readme = ReadMe()
        self.whenToAddBuildRules = .always
        self.generateXcodeProjectOnInit = GenerateXcodeProjectOnInit()
        self.regenerateXcodeProject = false
        self.repository = nil
        self.authorName = nil
        //self.authorContacts = []
        self.lockGenFiles = true
        self.verboseMode = false
        self.includeGeneratedFilesInXcodeProject = false
        self.includeSwiftLintInXcodeProject = .never
        self.autoInstallMissingPackages = false
        self.defaultPackageInitToolsVersion = nil
        self.defaultGitIgnore = nil
        self.gitIgnoreAdditions = nil
    }
}

extension DSwiftSettings: Codable {
    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        #if NO_DSWIFT_PARAMS
        self.swiftPath = DSwiftSettings.defaultSwiftPath
        #else
        self.swiftPath = try container.decodeIfPresent(String.self,
                                                       forKey: .swiftPath,
                                                       withDefaultValue: DSwiftSettings.defaultSwiftPath)
        #endif
        self.xcodeResourceSorting = try container.decodeIfPresent(XcodeProjectFileResourceSorting.self,
                                                                  forKey: .xcodeResourceSorting,
                                                                  withDefaultValue: .none)
        self.license = try container.decodeIfPresent(License.self,
                                                     forKey: .license,
                                                     withDefaultValue: .none)
        self.readme = try container.decodeIfPresent(ReadMe.self,
                                                    forKey: .readme,
                                                    withDefaultValue: ReadMe())
        self.whenToAddBuildRules = try container.decodeIfPresent(AddingBuildRulesRule.self,
                                                                 forKey: .whenToAddBuildRules,
                                                                 withDefaultValue: .always)
        self.generateXcodeProjectOnInit = try container.decodeIfPresent(GenerateXcodeProjectOnInit.self,
                                                                        forKey: .generateXcodeProjectOnInit,
                                                                        withDefaultValue: GenerateXcodeProjectOnInit())
        self.regenerateXcodeProject = try container.decodeIfPresent(Bool.self,
                                                                    forKey: .regenerateXcodeProject,
                                                                    withDefaultValue: false)
        self.repository = try container.decodeIfPresent(Repository.self,
                                                        forKey: .repository)
        self.authorName = try container.decodeIfPresent(String.self,
                                                        forKey: .authorName)
        self.lockGenFiles = try container.decodeIfPresent(Bool.self,
                                                          forKey: .lockGenFiels,
                                                          withDefaultValue: true)
        self.verboseMode = try container.decodeIfPresent(Bool.self,
                                                         forKey: .verboseMode,
                                                         withDefaultValue: false)
        self.includeGeneratedFilesInXcodeProject = try container.decodeIfPresent(Bool.self,
                                                                                 forKey: .includeGeneratedFilesInXcodeProject,
                                                                                 withDefaultValue: false)
        var swiftLint: Usability = .never
        if let useSwiftLint = try container.decodeIfPresent(Usability.self, forKey: .includeSwiftLintInXcodeProject) {
            swiftLint = useSwiftLint
        } else if let useSwiftLint = try container.decodeIfPresent(Bool.self, forKey: .includeSwiftLintInXcodeProjectIfAvailable), useSwiftLint {
            swiftLint = .whenAvailable
        }
        
        self.includeSwiftLintInXcodeProject = swiftLint
        
        #if AUTO_INSTALL_PACKAGES
        self.autoInstallMissingPackages = true
        #else
        self.autoInstallMissingPackages = try container.decodeIfPresent(Bool.self,
                                                                        forKey: .autoInstallMissingPackages,
                                                                        withDefaultValue: false)
        #endif
        
        self.defaultPackageInitToolsVersion = try container.decodeIfPresent(String.self,
                                                                            forKey: .defaultPackageInitToolsVersion)
        
        self.defaultGitIgnore = try container.decodeIfPresent(GitIgnoreFile.self,
                                                              forKey: .defaultGitIgnore)
        self.gitIgnoreAdditions = try container.decodeIfPresent(GitIgnoreFile.self,
                                                                forKey: .gitIgnoreAdditions)
        
    }
    
    /// Parse the DSwift Settings String
    /// - Parameters:
    ///   - string: The DSwift Settings String to parse
    ///   - encoding: The encoding of the DSwift Settings String
    public init(from string: String,
                encoding: String.Encoding = .utf8) throws {
        
        // Remove any comments
        let str = try JSONStringHelper.removeComments(json: string,
                                                      options: .removeEmptyLines,
                                                      commentBlocks: .pound, .doubleSlash, .slashStar)
        
        
        
        
        guard let dta = str.data(using: encoding) else {
            throw SettingsErrors.unableToConvertStringToData(string: str,
                                                             objectType: "Dswift Configuration Settings",
                                                             encoding: encoding)
        }
        self = try JSONDecoder().decode(DSwiftSettings.self, from: dta)
        
    }
    
    /// Open DSwift Settings File at the given URL
    /// - Parameters:
    ///   - url: The URL to the DSwift Settings File
    public init(from url: URL) throws {
        
        var encoding: String.Encoding = .utf8
        let str: String = try String(contentsOf: url.standardizedFileURL.resolvingSymlinksInPath(),
                                     foundEncoding: &encoding)
        
        try self.init(from: str,
                      encoding: encoding)
    }
    
    /// Open DSwift Settings File at the given URL
    /// - Parameters:
    ///   - path: The path to the DSwift Settings File
    public init(fromPath path: String) throws {
        try self.init(from: URL(fileURLWithPath: path))
    }
    
    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encode(self.swiftPath,
                             forKey: .swiftPath,
                             ifNot: DSwiftSettings.defaultSwiftPath)
        try container.encode(self.xcodeResourceSorting,
                             forKey: .xcodeResourceSorting)
        try container.encode(self.license,
                             forKey: .license,
                             ifNot: .none )
        try container.encode(self.readme,
                             forKey: .readme,
                             where: { return !$0.isEmpty })
        //if !self.readme.isEmpty { try container.encode(self.readme, forKey: .readme) }
        try container.encode(self.whenToAddBuildRules,
                             forKey: .whenToAddBuildRules,
                             ifNot: .always)
        try container.encode(self.generateXcodeProjectOnInit,
                             forKey: .generateXcodeProjectOnInit,
                             where: { return !$0.isEmpty })
        try container.encode(self.regenerateXcodeProject,
                             forKey: .regenerateXcodeProject )
        try container.encodeIfPresent(self.repository,
                                      forKey: .repository)
        try container.encodeIfPresent(self.authorName,
                                      forKey: .authorName)
        //try container.encodeToSingleOrArray(self.authorContacts, forKey: .authorContacts)
        try container.encode(self.verboseMode,
                             forKey: .verboseMode,
                             ifNot: false)
        try container.encode(self.includeGeneratedFilesInXcodeProject,
                             forKey: .includeGeneratedFilesInXcodeProject,
                             ifNot: false)
        try container.encode(self.includeSwiftLintInXcodeProject,
                             forKey: .includeSwiftLintInXcodeProject,
                             ifNot: .never)
        try container.encode(self.autoInstallMissingPackages,
                             forKey: .autoInstallMissingPackages,
                             ifNot: false)
        try container.encodeIfPresent(self.defaultPackageInitToolsVersion,
                                      forKey: .defaultPackageInitToolsVersion)
        try container.encodeIfPresent(self.defaultGitIgnore,
                                      forKey: .defaultGitIgnore)
        try container.encodeIfPresent(self.gitIgnoreAdditions,
                                      forKey: .gitIgnoreAdditions)
    }
}
extension DSwiftSettings.GenerateXcodeProjectOnInit: Codable {
    public init(from decoder: Decoder) throws {
        if let container = try? decoder.singleValueContainer() {
            let val = try container.decode(Bool.self)
            self.types = ["all": val]
        } else {
            var tps: [String: Bool] = [:]
            let container = try decoder.container(keyedBy: CodableKey.self)
            for key in container.allKeys {
                let v = try container.decode(Bool.self, forKey: key)
                var strKey = key.stringValue.lowercased()
                if strKey == "sysmod" { strKey = "system-module" }
                tps[strKey] = v
            }
            self.types = tps
        }
    }
    public func encode(to encoder: Encoder) throws {
        
        if let v = self.types["all"], self.types.count == 1 {
            var container = encoder.singleValueContainer()
            try container.encode(v)
        } else {
            var container = encoder.container(keyedBy: CodableKey.self)
            for (strKey, v) in self.types {
                try container.encode(v, forKey: CodableKey(stringValue: strKey.lowercased()))
            }
        }
        guard !self.isEmpty else { return }
    }
}

extension DSwiftSettings.License: Codable {
    public init(from decoder: Decoder) throws {
        let container = try decoder.singleValueContainer()
        let str = try container.decode(String.self)
        switch str {
            case "none": self = .none
            case "apache2_0": self = .apache2_0
            case "gnuGPL3_0": self = .gnuGPL3_0
            case "gnuAGPL3_0": self = .gnuAGPL3_0
            case "gnuLGPL3_0": self = .gnuLGPL3_0
            case "mozilla2_0": self = .mozilla2_0
            case "mit": self = .mit
            case "unlicense": self = .unlicense
            default:
                if str.hasPrefix("/") || str.hasPrefix(".") || str.hasPrefix("~") {
                    self = .file(URL(fileURLWithPath: NSString(string: str).expandingTildeInPath))
                } else {
                    if let url = URL(string: str) {
                        self = .file(url)
                    } else {
                        print("Invalid license url '\(str)'.")
                        self = .none
                    }
                    
                }
        }
    }
    public func encode(to encoder: Encoder) throws {
        var container = encoder.singleValueContainer()
        switch self {
            case .none: try container.encode("none")
            case .apache2_0: try container.encode("apache2.0")
            case .gnuGPL3_0: try container.encode("gnuGPL3.0")
            case .gnuAGPL3_0: try container.encode("gnuAGPL3.0")
            case .gnuLGPL3_0: try container.encode("gnuLGPL3.0")
            case .mozilla2_0: try container.encode("mozilla2_0")
            case .mit: try container.encode("mit")
            case .unlicense: try container.encode("unlicense")
            case .file(let url): try container.encode(url)
        }
    }
    
}
 
extension DSwiftSettings {
    /// Write license to given location
    public func writeLicense(to url: URL,
                             console: Console = .null) throws {
        switch self.license {
            case .file(let licenseURL):
                console.print("Creating License.md")
                if licenseURL.isFileURL {
                    let path = licenseURL.path
                    let lURL = URL(fileURLWithPath: NSString(string: path).expandingTildeInPath)
                    try FileManager.default.copyItem(at: lURL, to: url)
                } else {
                    let dta = try Data(contentsOf: licenseURL)
                    try dta.write(to: url, options: .atomic)
                }
            case .apache2_0:
                console.print("Creating License.md")
                try Licenses.apache2_0().write(to: url, atomically: true, encoding: .utf8)
            case .gnuGPL3_0:
                console.print("Creating License.md")
                try Licenses.gnuGPL3_0().write(to: url, atomically: true, encoding: .utf8)
            case .gnuAGPL3_0:
                console.print("Creating License.md")
                try Licenses.gnuAGPL3_0().write(to: url, atomically: true, encoding: .utf8)
            case .gnuLGPL3_0:
                console.print("Creating License.md")
                try Licenses.gnuLGPL3_0().write(to: url, atomically: true, encoding: .utf8)
            case .mozilla2_0:
                console.print("Creating License.md")
                try Licenses.mozilla2_0().write(to: url, atomically: true, encoding: .utf8)
            case .mit:
                console.print("Creating License.md")
                try Licenses.mit(settings: self).write(to: url, atomically: true, encoding: .utf8)
            case .unlicense:
                console.print("Creating License.md")
                try Licenses.unlicense().write(to: url, atomically: true, encoding: .utf8)
            default: break
        }
    }
}

extension DSwiftSettings.Contact: Codable {
    public init(from decoder: Decoder) throws {
        let container = try decoder.singleValueContainer()
        if let address = try? container.decode([String].self) {
            self = .address(address.joined(separator: "\n"))
        } else {
            let str = try container.decode(String.self)
            if str.hasPrefix("mailto:") {
                self = .email(String(str.suffix(from: str.index(str.startIndex, offsetBy: "mailto:".count))))
            } else if str.hasPrefix("tel:") {
                self = .phone(String(str.suffix(from: str.index(str.startIndex, offsetBy: "tel:".count))))
            } else {
                self = .url(str)
            }
        }
    }
    public func encode(to encoder: Encoder) throws {
        var container = encoder.singleValueContainer()
        if self.isEmail { try container.encode("mailto:" + self.string) }
        else if self.isPhone { try container.encode("tel:" + self.string) }
        else if self.isAddress { try container.encode(self.string.split(separator: "\n").map(String.init)) }
        else { try container.encode(self.string) }
    }
}
extension DSwiftSettings.Repository: Codable {
    public init(from decoder: Decoder) throws {
        if let container = try? decoder.singleValueContainer() {
            self.serviceName = nil
            self.serviceURL = try container.decode(URL.self)
            self.repositoryName = self.serviceURL.lastPathComponent
        } else {
            let container = try decoder.container(keyedBy: CodingKeys.self)
            self.serviceName = try container.decodeIfPresent(String.self, forKey: .serviceName)
            self.serviceURL = try container.decode(URL.self, forKey: .serviceURL)
            self.repositoryName = try container.decodeIfPresent(String.self, forKey: .repositoryName)
        }
    }
    public func encode(to encoder: Encoder) throws {
        if (self.repositoryName == nil && self.serviceName == nil) ||
            ( self.serviceName == nil && self.repositoryName == self.serviceURL.lastPathComponent) {
            var container = encoder.singleValueContainer()
            try container.encode(self.serviceURL)
        } else {
            var container = encoder.container(keyedBy: CodingKeys.self)
            try container.encodeIfPresent(self.serviceName, forKey: .serviceName)
            try container.encode(self.serviceURL, forKey: .serviceURL)
            try container.encodeIfPresent(self.repositoryName, forKey: .repositoryName)
        }
    }
}

extension DSwiftSettings.ReadMe.ReadMeType: Codable {
    public enum Errors: Error {
        case invalidURL(String)
    }
    private static func decodeURL(_ string: String) -> URL? {
        if string.hasPrefix("/") || string.hasPrefix(".") || string.hasPrefix("~") {
            return URL(fileURLWithPath: NSString(string: string).expandingTildeInPath)
        } else { return URL(string: string) }
    }
    
    public init(from decoder: Decoder) throws {
        let container = try decoder.singleValueContainer()
        let str = try container.decode(String.self)
        if str == "generated" { self = .generated }
        else if let url = DSwiftSettings.ReadMe.ReadMeType.decodeURL(str) {
            self = .url(url)
        } else {
            throw Errors.invalidURL(str)
        }
    }
    public func encode(to encoder: Encoder) throws {
        var container = encoder.singleValueContainer()
        switch self {
            case .generated: try container.encode("generated")
            case .url(let url): try container.encode(url)
        }
    }
}

extension DSwiftSettings.ReadMe: Codable {
    
    public init(from decoder: Decoder) throws {
        if let container = try? decoder.singleValueContainer() {
            let val = try container.decode(DSwiftSettings.ReadMe.ReadMeType.self)
            self.types = ["all": val]
        } else {
            let container = try decoder.container(keyedBy: CodableKey.self)
            var tps: [String: ReadMeType] = [:]
            for key in container.allKeys {
                var strKey = key.stringValue.lowercased()
                if strKey == "sysmod" { strKey = "system-module" }
                let val = try container.decode(DSwiftSettings.ReadMe.ReadMeType.self, forKey: key)
                tps[strKey] = val
            }
            self.types = tps
        }
    }
    
    
    public func encode(to encoder: Encoder) throws {
        if let allVal = self.types["all"] {
            var container = encoder.singleValueContainer()
            try container.encode(allVal)
        } else {
            var container = encoder.container(keyedBy: CodableKey.self)
            for (strKey, val) in self.types {
                try container.encode(val,
                                     forKey: CodableKey(stringValue: strKey.lowercased()))
            }
        }
    }
    
}
extension DSwiftSettings {
    /// Write readme file to specific location
    public func writeReadMe(to url: URL,
                            for modType: String,
                            withName name: String,
                            includeDSwiftMessage dswiftMessage: Bool = true,
                            dswiftInfo: DSwiftInfo,
                            currentProjectURL: URL,
                            console: Console = .null) throws {
        do {
        
            
            guard let gen = try self.self.readme.generateReadMe(for: modType,
                                                                withName: name,
                                                                includeDSwiftMessage: dswiftMessage,
                                                                dswiftInfo: dswiftInfo,
                                                                currentProjectURL: currentProjectURL,
                                                                authorName: self.authorName,
                                                                license: self.license,
                                                                console: console) else {
                return
            }
            
            switch gen {
                case .copyFrom(url: let rURL, content: _, encoding: _):
                    console.print("Replacing README.md with \(rURL)", object: self)
                case .replaceWith(content: _, encoding: _):
                    console.print("Replacing README.md with generated file", object: self)
            }
            
            try gen.content.write(to: url,
                                  atomically: true,
                                  encoding: gen.encoding)
            
        } catch let err as ReadMe.GenerateReadMeError {
            console.printError(err.description, object: self)
        }
    }
}


// MARK: - Equetable Settings

extension DSwiftSettings.XcodeProjectFileResourceSorting: Equatable {
    public static func ==(lhs: DSwiftSettings.XcodeProjectFileResourceSorting,
                          rhs: DSwiftSettings.XcodeProjectFileResourceSorting) -> Bool {
        switch (lhs, rhs) {
            case (.none, .none): return true
            case (.sorted, .sorted): return true
            default: return false
        }
    }
}

extension DSwiftSettings.GenerateXcodeProjectOnInit: Equatable {
    public static func ==(lhs: DSwiftSettings.GenerateXcodeProjectOnInit,
                          rhs: DSwiftSettings.GenerateXcodeProjectOnInit) -> Bool {
        guard lhs.count == rhs.count else {
            return false
        }
        
        for (k,v) in lhs {
            guard let rhsV = rhs[k] else {
                return false
            }
            if v != rhsV {
                return false
            }
        }
        return true
    }
}

extension DSwiftSettings.Contact: Equatable {
    public static func ==(lhs: DSwiftSettings.Contact,
                          rhs: DSwiftSettings.Contact) -> Bool {
        switch (lhs, rhs) {
            case (.email(let lhsV), .email(let rhsV)):
                return lhsV == rhsV
            case (.address(let lhsV), .address(let rhsV)):
                return lhsV == rhsV
            case (.phone(let lhsV), .phone(let rhsV)):
                return lhsV == rhsV
            case (.url(let lhsV), .url(let rhsV)):
                return lhsV == rhsV
            default: return false
        }
    }
}

extension DSwiftSettings.License: Equatable {
    public static func ==(lhs: DSwiftSettings.License,
                          rhs: DSwiftSettings.License) -> Bool {
        switch (lhs, rhs) {
            case (.none, .none):
                return true
            case (.apache2_0, .apache2_0):
                return true
            case (.gnuGPL3_0, .gnuGPL3_0):
                return true
            case (.gnuAGPL3_0, .gnuAGPL3_0):
                return true
            case (.gnuLGPL3_0, .gnuLGPL3_0):
                return true
            case (.mozilla2_0, .mozilla2_0):
                return true
            case (.mit, .mit):
                return true
            case (.unlicense, .unlicense):
                return true
            case (.file(let lhsV), .file(let rhsV)):
                return lhsV == rhsV
            default:
                return false
        }
    }
}

extension DSwiftSettings.Repository: Equatable {
    public static func ==(lhs: DSwiftSettings.Repository,
                          rhs: DSwiftSettings.Repository) -> Bool {
        
        return lhs.serviceName == rhs.serviceName &&
               lhs.serviceURL == rhs.serviceURL &&
               lhs.repositoryName == rhs.repositoryName
    }
}

extension DSwiftSettings.ReadMe.ReadMeType: Equatable {
    public static func ==(lhs: DSwiftSettings.ReadMe.ReadMeType,
                          rhs: DSwiftSettings.ReadMe.ReadMeType) -> Bool {
        switch (lhs, rhs) {
            case (.generated, .generated):
                return true
            case (.url(let lhsV), .url(let rhsV)):
                return lhsV == rhsV
            default:
                return false
        }

    }
}

extension DSwiftSettings.ReadMe: Equatable {
    public static func ==(lhs: DSwiftSettings.ReadMe,
                          rhs: DSwiftSettings.ReadMe) -> Bool {
        
        guard lhs.count == rhs.count else {
            return false
        }
        
        for (k,v) in lhs {
            guard let rhsV = rhs[k] else {
                return false
            }
            if v != rhsV {
                return false
            }
        }
        return true
    }
}

extension DSwiftSettings.AddingBuildRulesRule: Equatable {
    public static func ==(lhs: DSwiftSettings.AddingBuildRulesRule,
                          rhs: DSwiftSettings.AddingBuildRulesRule) -> Bool {
        switch (lhs, rhs) {
            case (.always, .always):
                return true
            case (.whenNeeded, .whenNeeded):
                return true
            default:
                return false
        }

    }
}

extension DSwiftSettings.Usability: Equatable {
    public static func ==(lhs: DSwiftSettings.Usability,
                          rhs: DSwiftSettings.Usability) -> Bool {
        switch (lhs, rhs) {
            case (.always, .always):
                return true
            case (.whenAvailable, .whenAvailable):
                return true
            case (.never, .never):
                return true
            default:
                return false
        }

    }
}

extension DSwiftSettings: Equatable {
    public static func ==(lhs: DSwiftSettings,
                          rhs: DSwiftSettings) -> Bool {
        return lhs.swiftPath == rhs.swiftPath &&
        lhs.xcodeResourceSorting == rhs.xcodeResourceSorting &&
        lhs.license == rhs.license &&
        lhs.readme == rhs.readme &&
        lhs.whenToAddBuildRules == rhs.whenToAddBuildRules &&
        lhs.generateXcodeProjectOnInit == rhs.generateXcodeProjectOnInit &&
        lhs.regenerateXcodeProject == rhs.regenerateXcodeProject &&
        lhs.repository == rhs.repository &&
        lhs.authorName == rhs.authorName &&
        //lhs.authorContacts.elementsEqual(rhs.authorContacts) &&
        lhs.lockGenFiles == rhs.lockGenFiles &&
        lhs.verboseMode == rhs.verboseMode &&
        lhs.includeGeneratedFilesInXcodeProject == rhs.includeGeneratedFilesInXcodeProject &&
        lhs.includeSwiftLintInXcodeProject == rhs.includeSwiftLintInXcodeProject &&
        lhs.autoInstallMissingPackages == rhs.autoInstallMissingPackages &&
        lhs.defaultPackageInitToolsVersion == rhs.defaultPackageInitToolsVersion &&
        lhs.defaultGitIgnore == rhs.defaultGitIgnore &&
        lhs.gitIgnoreAdditions == rhs.gitIgnoreAdditions
    }
}
